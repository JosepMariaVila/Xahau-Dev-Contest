...
From now on we will use thethe Loan to Value terminology. In Loan-to-Value (LTV) terms: a loan (5 XAU USD) divided by XAH value collateralized ($10), which results in 0'5, then multiplied by 100, we have a 100% ratio. So, we have an initial loan with a 50% ratio. The position gets liquidated if the LTV ratio goes just above 83% (if the denominator-value goes down the resulting ratio increases). A 33% secure margin from the initial position (50%) until the liquidation ratio starts (83%) is enforced by the protocol.
